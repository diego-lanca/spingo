...
export class MyElement extends LitElement {
  ...
  async spinSlot() {
    try {
      ...
      console.log("Símbolos:", result.symbols);
      console.log("Ganhou?", result.is_win);
      console.log("Valor ganho:", result.win_amount);
  ...
  greet() {
    let thisName = (this.shadowRoot?.getElementById("name") as HTMLInputElement)
      ?.value;
    if (thisName) {
    ...
    return html`
      <main>
        ...
        <div class="input-box" id="input">
          ...
          <button @click=${this.spinSlot} class="btn">Spin</button>
        </div>
      </main>
    `;
  }
